...
def spatial_smooth(
    ...
    """Smooth cell labels based on spatial k-nearest neighbors.

    This function assigns a smoothed label to each cell by taking the most frequent 
    label among its k-nearest neighbors based on spatial coordinates. Embeddings are 
    added for completeness but not used in the smoothing.

    Args:
        adata (AnnData): 
            Annotated data object containing spatial coordinates, embeddings, and cell labels.
        k (int, optional): 
            Number of nearest neighbors to consider for smoothing. Defaults to 10.
        spatial_key (str, optional): 
            Key in `adata.obsm` for spatial coordinates (e.g., 'spatial'). Defaults to 'spatial'.
        label_key (str, optional): 
            Key in `adata.obs` for cell labels (e.g., 'leiden'). Defaults to 'leiden'.
        emb_key (str, optional): 
            Key in `adata.obsm` for embeddings (e.g., PCA). Defaults to 'X_emb_smooth'.
        region_key (str, optional): 
            Key prefix to store the smoothed result in `adata.obs`. Defaults to 'region'.

    Returns:
        AnnData: 
            Updated AnnData object with smoothed labels stored in 
            `adata.obs[f'{region_key}_smooth']`.
    """
...
# ------------------------------------------------------------------------------------------------
# Bin data spatially and compute gene expression per bin (For image-based data)
# ------------------------------------------------------------------------------------------------
def bin_data(
    data: pd.DataFrame,
    n_points: int = 20,
    ...
    Args:
        data (pd.DataFrame): 
            Input DataFrame containing spatial coordinates, gene names, and transcript IDs.
        n_points (int, optional): 
            Number of random points sampled to estimate bin size. Defaults to 20.
        min_transcripts (int, optional): 
            Minimum number of transcripts required per bin. Defaults to 50.
        ...
        return_pixel (bool, optional): 
            Whether to return the original data with bin assignments. Defaults to False.
...
# ------------------------------------------------------------------------------------------------
# Build cell-by-gene expression matrix and centroid coordinate matrix
# ------------------------------------------------------------------------------------------------
...
def plot_segmented_cells(
    ...
    Args:
        ...
        fig_title (str, optional): 
            Title to be shown on the plot. Defaults to "Segmented Cells (ScaleFlow)".
        h (float, optional): 
